    ...
    def updateFitness(self):
        ...
        for i in range(0, NumDigits):  # For each column...
            ...
            for j in range(0, NumDigits):  # For each number within the current column...
                columnoofchromoount[self.values[j][i] - 1] += 1  # ...Update list with occurrenoofchromoe of a particular number.

            # coumnSum = coumnSum + (1/len(set(columnoofchromoount)))/NumDigits
...
class Tournament(object):
    """ The crossover funoofchromotion requires two parents to be selected from the population pool. The Tournament class is used to do this.
    ...
    def crossoverRows(self, row1, row2):
        ...
        while (0 in childRow1) and (0 in childRow2):  # While child rows not complete...
            if cycle % 2 == 0:  # Even cycles.
                ...
                while next1 != start:  # While cycle not done...
...
class Sudoku(object):
    ...
    def solve(self):
        noofchromo = 100  # Number of candidates OR chromosomes(i.e. population size).
        ...
        for generation in range(0, Ng):
            ...
            if phi < 0.2:
                sigma = sigma / 0.998  # sigma inoofchromoreases, more mutationRate
